...
-- 音乐喜欢表（用于收藏/喜欢单曲）
-- key 建议格式：
-- - 默认：`${source}:${song_id}`（与 generateStorageKey(source, id) 一致）
-- - 若需要“歌手名+歌名”去重（如网易云喜欢导入）：`${source}:${normalize(artist)}::${normalize(title)}`
...
CREATE INDEX IF NOT EXISTS idx_music_likes_username ON music_likes(username);
-- 音乐喜欢：用户名+键值的复合索引，用于快速查找特定歌曲
CREATE INDEX IF NOT EXISTS idx_music_likes_username_key ON music_likes(username, key);
-- 音乐喜欢：用户名+保存时间的复合索引，用于按时间排序的查询
CREATE INDEX IF NOT EXISTS idx_music_likes_username_save_time ON music_likes(username, save_time DESC);
-- 音乐喜欢：用户名+来源+歌曲ID 的复合索引，便于按来源查找/去重
CREATE INDEX IF NOT EXISTS idx_music_likes_username_source_song_id ON music_likes(username, source, song_id);

```

文档更新：
• D1 初始化.md - 添加了 nav_categories 和 nav_sites 表结构及索引
• D1 初始化.md - 添加了 music_likes（音乐喜欢）表结构及索引
